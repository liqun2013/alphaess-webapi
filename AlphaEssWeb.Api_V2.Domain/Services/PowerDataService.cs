...
namespace AlphaEssWeb.Api_V2.Domain.Services
{
	public class PowerDataService : IPowerDataService
	{
		...
		private readonly IEntityRepository<PowerData, Guid> _powerRepository;
		...
		/// <returns></returns>
		public OperationResult<PaginatedList<PowerData>> GetLastPowerData(string api_Account, long timeStamp, string sign, string sn, string token, string ipAddress)
		{
			Guid key = Guid.Empty;
			...
			{
				return new OperationResult<PaginatedList<PowerData>>(OperationCode.Error_LoginFailed);
			}

			if (userCache.ipAddress != ipAddress)
			{
                var ul = new SYS_LOG { Key = Guid.NewGuid(), CREATE_DATETIME = DateTime.Now, THREAD = OperationCode.Error_UserChangesDevice.ToString(), LEVEL = LogLevel.ERROR.ToString(), LOGGER = "Services.PowerDataService", MESSAGE = "用户更换设备", EXCEPTION = "ipAddress_1:" + userCache.ipAddress + ",ipAddress:" + ipAddress, CREATE_ACCOUNT = "", LOG_CONTENT = "" };
                _sysLogService.Add(ul);
                _sysLogService.Save();

                return new OperationResult<PaginatedList<PowerData>>(OperationCode.Error_UserChangesDevice);
			}
			...
			if (!checkSignForLastPowerData(api_Account, timeStamp, sign, sn, token))
			{
                string secretKey = _cryptoService.GetSecretKey(api_Account);
                var ul = new SYS_LOG { Key = Guid.NewGuid(), CREATE_DATETIME = DateTime.Now, THREAD = OperationCode.Error_Sign.ToString(), LEVEL = LogLevel.ERROR.ToString(), LOGGER = "Services.PowerDataService", MESSAGE = "签名错误", EXCEPTION = "api_Account:" + api_Account + ",timeStamp:" + timeStamp + ",sign:" + sign + ",token:" + token + ", sn:" + sn + ",ipAddress:" + ipAddress + ",SecretKey:" + secretKey, CREATE_ACCOUNT = "", LOG_CONTENT = "" };
                _sysLogService.Add(ul);
                _sysLogService.Save();
			...
			{
                var ul = new SYS_LOG { Key = Guid.NewGuid(), CREATE_DATETIME = DateTime.Now, THREAD = OperationCode.Error_TimeStamp.ToString(), LEVEL = LogLevel.ERROR.ToString(), LOGGER = "Services.PowerDataService", MESSAGE = "时间戳错误", EXCEPTION = "api_Account:" + api_Account + ",timeStamp:" + timeStamp + ",sign:" + sign + ",token:" + token + ", sn:" + sn + ",ipAddress:" + ipAddress, CREATE_ACCOUNT = "", LOG_CONTENT = "" };
                _sysLogService.Add(ul);
                _sysLogService.Save();
			...
			{
                var ul = new SYS_LOG { Key = Guid.NewGuid(), CREATE_DATETIME = DateTime.Now, THREAD = OperationCode.Error_ApiAccountNotExist.ToString(), LEVEL = LogLevel.ERROR.ToString(), LOGGER = "Services.PowerDataService", MESSAGE = "api账号不存在", EXCEPTION = "api_Account:" + api_Account + ",timeStamp:" + timeStamp + ",sign:" + sign + ",token:" + token + ", sn:" + sn + ",ipAddress:" + ipAddress, CREATE_ACCOUNT = "", LOG_CONTENT = "" };
                _sysLogService.Add(ul);
                _sysLogService.Save();
			...
			int totalCount = 0;
			PaginatedList<PowerData> powerdataList = new PaginatedList<PowerData>(pIndex, pSize, null);
			...
			if (!string.IsNullOrWhiteSpace(sn))
			{
				...
				{
                    var ul = new SYS_LOG { Key = Guid.NewGuid(), CREATE_DATETIME = DateTime.Now, THREAD = OperationCode.Error_SNNotExist.ToString(), LEVEL = LogLevel.ERROR.ToString(), LOGGER = "Services.PowerDataService", MESSAGE = "sn不存在", EXCEPTION = "api_Account:" + api_Account + ",timeStamp:" + timeStamp + ",sign:" + sign + ",token:" + token + ", sn:" + sn + ",ipAddress:" + ipAddress, CREATE_ACCOUNT = "", LOG_CONTENT = "" };
                    _sysLogService.Add(ul);
                    _sysLogService.Save();

                    return new OperationResult<PaginatedList<PowerData>>(OperationCode.Error_SNNotExist);
				}
				...
				if (systems.Where(x => x.SYS_SN == sn).Count() == 0)
				{
                    ...
                    var ul = new SYS_LOG { Key = Guid.NewGuid(), CREATE_DATETIME = DateTime.Now, THREAD = OperationCode.Error_NoPermissionsToQuery.ToString(), LEVEL = LogLevel.ERROR.ToString(), LOGGER = "Services.PowerDataService", MESSAGE = "用户无此权限,调取用户:"+ username, EXCEPTION = "api_Account:" + api_Account + ",timeStamp:" + timeStamp + ",sign:" + sign + ",token:" + token + ", sn:" + sn + ",ipAddress:" + ipAddress, CREATE_ACCOUNT = "", LOG_CONTENT = "" };
                    _sysLogService.Add(ul);
                    _sysLogService.Save();

                    return new OperationResult<PaginatedList<PowerData>>(OperationCode.Error_NoPermissionsToQuery);
				}

				powerdataList = GetLastPowerDataBySn(api_Account, timeStamp, sign, sn, pIndex, pSize, out totalCount);
			}
			else
			{
				powerdataList = GetLastPowerDataByUsername(api_Account, timeStamp, sign, systems, pIndex, pSize, out totalCount);

			}

			powerdataList.TotalCount = totalCount;
			powerdataList.TotalPageCount = (int)Math.Ceiling(powerdataList.TotalCount / (double)pSize);
			var pwdata = new OperationResult<PaginatedList<PowerData>>(OperationCode.Success);
			pwdata.Entity = powerdataList;

			return pwdata;
		}

		private PaginatedList<PowerData> GetLastPowerDataByUsername(string api_Account, long timeStamp, string sign, IList<VT_SYSTEM> systems, int pIndex, int pSize, out int totalCount)
		{
			...
			result = _powerRepository.GetAll()
				.OrderByDescending(o => o.UploadTime)
				.GroupBy(p => new { p.Sn })
				.Select(g => g.FirstOrDefault())
				.ToList();

			result = result.Where(x => lst.Contains(x.Sn)).ToList();
		...
		private bool checkSignForLastPowerData(string api_Account, long timeStamp, string sign, string sn, string token)
		{
